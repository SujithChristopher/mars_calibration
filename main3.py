            ...
            if result.returncode == 0:
                ...
                if result.returncode == 0:
                    ...
                    if upload_type == "calibration":
                        self.current_step = 2
                        self.log_signal.emit("✓ Step 1 completed! Now connect to serial and calibrate.")
                    elif upload_type == "firmware":
                        self.current_step = 4
                        self.log_signal.emit("✓ All steps completed! Load cell is ready to use.")
                    
                    # Update UI in main thread
                    QTimer.singleShot(100, self.update_step_status)
