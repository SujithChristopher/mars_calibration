...
class LoadCellCalibrationGUI(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # Reader Code Section
        reader_group = QGroupBox("Reader Code")
        reader_layout = QVBoxLayout(reader_group)
        
        # Reader file selection
        reader_file_layout = QHBoxLayout()
        self.reader_file_edit = QLineEdit()
        self.reader_file_edit.setPlaceholderText("Select reader.ino file...")
        self.browse_reader_button = QPushButton("Browse Reader")
        self.browse_reader_button.clicked.connect(self.browse_reader_file)
        reader_file_layout.addWidget(self.reader_file_edit)
        reader_file_layout.addWidget(self.browse_reader_button)
        reader_layout.addLayout(reader_file_layout)
        
        # Reader control buttons
        reader_button_layout = QVBoxLayout()
        
        self.update_reader_button = QPushButton("Update Reader with Cal Factor")
        self.update_reader_button.clicked.connect(self.update_reader_code)
        self.update_reader_button.setEnabled(False)
        reader_button_layout.addWidget(self.update_reader_button)
        
        self.upload_reader_button = QPushButton("Upload Reader to Arduino")
        self.upload_reader_button.clicked.connect(self.upload_reader_code)
        self.upload_reader_button.setEnabled(False)
        reader_button_layout.addWidget(self.upload_reader_button)
        
        reader_layout.addLayout(reader_button_layout)
        
        # Calibration factor input for manual override
        manual_cal_layout = QHBoxLayout()
        manual_cal_layout.addWidget(QLabel("Manual Cal Factor:"))
        self.manual_cal_spin = QDoubleSpinBox()
        self.manual_cal_spin.setRange(-999999.0, 999999.0)
        self.manual_cal_spin.setValue(1500.0)
        self.manual_cal_spin.setDecimals(2)
        manual_cal_layout.addWidget(self.manual_cal_spin)
        
        self.use_manual_button = QPushButton("Use Manual")
        self.use_manual_button.clicked.connect(self.use_manual_calibration)
        manual_cal_layout.addWidget(self.use_manual_button)
        reader_layout.addLayout(manual_cal_layout)
    ...
    def browse_reader_file(self):
        """Browse for reader.ino file"""
        from PySide6.QtWidgets import QFileDialog
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Select Reader Arduino File", "", "Arduino Files (*.ino)")
        if file_path:
            self.reader_file_edit.setText(file_path)
            self.update_reader_button.setEnabled(True)
            
    def use_manual_calibration(self):
        """Use manually entered calibration factor"""
        manual_cal = self.manual_cal_spin.value()
        self.current_calibration_factor = manual_cal
        self.cal_factor_label.setText(f"Calibration Factor: {manual_cal:.2f}")
        self.log_message(f"Using manual calibration factor: {manual_cal}")
        
        # Enable reader upload if file is selected
        if self.reader_file_edit.text():
            self.upload_reader_button.setEnabled(True)
            
    def update_reader_code(self):
        """Update reader.ino file with current calibration factor"""
        if not self.reader_file_edit.text():
            QMessageBox.warning(self, "Warning", "Please select a reader.ino file first!")
            return
            
        if self.current_calibration_factor == 1.0:
            QMessageBox.warning(self, "Warning", "No calibration factor available. Please calibrate first or use manual factor!")
            return
            
        try:
            reader_file_path = self.reader_file_edit.text()
            
            # Read the original reader file
            with open(reader_file_path, 'r') as file:
                content = file.read()
            ...
            if pattern in content or 'calibration_factor' in content:
                # Create backup
                backup_path = reader_file_path + ".backup"
                with open(backup_path, 'w') as backup_file:
                    backup_file.write(content)
                
                # Write updated content
                with open(reader_file_path, 'w') as file:
                    file.write(updated_content)
                
                self.log_message(f"Updated reader.ino with calibration factor: {self.current_calibration_factor:.2f}")
                self.log_message(f"Backup saved as: {backup_path}")
                self.upload_reader_button.setEnabled(True)
                
                QMessageBox.information(self, "Success", 
                    f"Reader code updated with calibration factor: {self.current_calibration_factor:.2f}\n"
                    f"Backup saved as: {backup_path}")
            else:
                QMessageBox.warning(self, "Warning", "Could not find calibration_factor line in reader file!")
                
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Failed to update reader file: {str(e)}")
            self.log_message(f"Error updating reader file: {str(e)}")
    ...
    def upload_reader_code(self):
        """Upload reader.ino code using arduino-cli"""
        if not self.reader_file_edit.text():
            QMessageBox.warning(self, "Warning", "Please select a reader.ino file first!")
            return
        ...
        if self.is_connected:
            self.log_message("Disconnecting serial for reader upload...")
            self.disconnect_serial()
            time.sleep(1)
        ...
        selected_port = self.port_combo.currentText()
        selected_board = self.board_combo.currentText()
        selected_file = self.reader_file_edit.text()
        
        self.log_message(f"Starting reader upload to {selected_port}")
        
        # Run upload in separate thread
        threading.Thread(target=self._upload_reader_thread, args=(selected_file, selected_board, selected_port), daemon=True).start()
    ...
    def _upload_reader_thread(self, sketch_path, board, port):
        """Upload reader thread function"""
        try:
            self.log_signal.emit(f"Reader Upload parameters:")
            self.log_signal.emit(f"  File: {sketch_path}")
            self.log_signal.emit(f"  Board: {board}")
            self.log_signal.emit(f"  Port: {port}")
            self.log_signal.emit(f"  Calibration Factor: {self.current_calibration_factor:.2f}")
            ...
            if "mbed_nano" in board:
                self.log_signal.emit("Checking Arduino Nano core installation...")
                core_install_cmd = f'"{arduino_cli_path}" core install arduino:mbed_nano'
                subprocess.run(core_install_cmd, shell=True, capture_output=True, text=True)
            ...
            # Execute commands
            self.log_signal.emit("Compiling reader sketch...")
            result = subprocess.run(compile_cmd, shell=True, capture_output=True, text=True)
            
            if result.returncode == 0:
                self.log_signal.emit("Reader compilation successful!")
                self.log_signal.emit(f"Uploading reader to {port}...")
                
                result = subprocess.run(upload_cmd, shell=True, capture_output=True, text=True)
                
                if result.returncode == 0:
                    self.log_signal.emit("Reader upload successful!")
                    self.log_signal.emit(f"Reader programmed on {port} with calibration factor {self.current_calibration_factor:.2f}")
                else:
                    self.log_signal.emit(f"Reader upload failed: {result.stderr}")
                    if "mbed_nano" in board:
                        self.log_signal.emit("Note: Make sure to double-press the reset button on Nano 33 BLE to enter bootloader mode")
            else:
                self.log_signal.emit(f"Reader compilation failed: {result.stderr}")
                
        except Exception as e:
            self.log_signal.emit(f"Reader upload error: {str(e)}")
        finally:
            # Hide progress bar and re-enable button
            self.progress_bar.setVisible(False)
            self.upload_reader_button.setEnabled(True)
        ...
        if "calibration value has been set to:" in data.lower():
            try:
                ...
                if len(parts) > 1:
                    ...
                    # Enable reader update if file is selected
                    if self.reader_file_edit.text():
                        self.upload_reader_button.setEnabled(True)
                    
                    self.log_message(f"Calibration factor updated: {cal_factor:.2f}")
            except:
                pass
